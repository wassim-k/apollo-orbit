...
export async function generateApiDocs(): Promise<DeclarationReflection> {
  const app = await Application.bootstrapWithPlugins({
    entryPoints: [
      ...
      toPosixPath(path.resolve(__dirname, '../../packages/angular/src/signals/types.ts')),
      toPosixPath(path.resolve(__dirname, '../../packages/angular/src/signals/query.ts')),
      toPosixPath(path.resolve(__dirname, '../../packages/angular/src/signals/singleQuery.ts')),
