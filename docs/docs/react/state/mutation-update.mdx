...
## Overview
`mutationUpdate` is analogous to the <Link to="https://www.apollographql.com/docs/react/data/mutations/#the-update-function">`update` option passed to `useMutation`</Link>.  
...
Now, when a component calls `useMutation(AddBookDocument)`, the above mutation update functions are automatically executed and the UI displaying books and author books is updated.
