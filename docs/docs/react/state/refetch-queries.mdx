...
## Overview
`refetchQueries` is analogous to <Link to="https://www.apollographql.com/docs/react/data/mutations/#mutationhookoptions-interface-refetchqueries">`refetchQueries` option passed to `useMutation`</Link>.  
...
export const bookState = state(descriptor => descriptor
  ...
  .mutationUpdate(AddBookDocument, (cache, info) => {
    ...
    if (!addBook) return;
    cache.updateQuery(BooksDocument, query => query ? { books: [...query.books, addBook] } : query);
  })
  // code-remove-end
);
