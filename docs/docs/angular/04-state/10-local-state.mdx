...
Saving this file will trigger the codegen of `Theme` and `ThemeName` types in *`graphql/types.ts`* file as per our setup.
...
:::note
`__typename` is explicitly selected here because the state's `onInit` writes a full `Theme` object to the cache.  
As of `typescript-operations` v6, `__typename` is only included in the generated types when it's explicitly selected in the operation.
:::
...
```typescript title="states/theme/theme.state.ts"
import { state } from '@apollo-orbit/angular/state';
import { ThemeName, THEME_QUERY } from '../../graphql';
