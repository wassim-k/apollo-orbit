...
**Apollo Orbit** aims to bridge the gap between **Apollo Client** and traditional state management libraries like NGXS, NgRx or Redux.

It does so by providing the following features:
<StateFeatures />

### Opt-in
**Apollo Orbit** state module is optional and is only included in the final bundle if you opt-in by importing `@apollo-orbit/angular/state` entry point.
