...
| `from` | `TFrom \| (() => TFrom)` | An object containing a `__typename` and primary key fields<br />(such as `id`) identifying the entity object from which the fragment will<br />be retrieved, or a `{ __ref: "..." }` reference, or a `string` ID<br />(uncommon). |
