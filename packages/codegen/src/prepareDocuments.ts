...
import { concatAST, DocumentNode, FragmentDefinitionNode, Kind } from 'graphql';
...
export function prepareDocuments(documents: Array<Types.DocumentFile>, externalFragments: Array<LoadedFragment> | undefined): [Array<LoadedFragment>, DocumentNode] {
  ...
  const allFragments: Array<LoadedFragment> = [
    ...(allAst.definitions.filter(d => d.kind === Kind.FRAGMENT_DEFINITION) as Array<FragmentDefinitionNode>).map(fragmentDef => ({
      node: fragmentDef,
